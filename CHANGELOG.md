...
- 原版僵尸替换安装改为只字段级合并 `animationId`，保留已有生命、攻击、防具和其他扩展字段，并停止生成 `custom_zombies.generated.jsonc`。真正新增僵尸只生成明确标注 `runtimeStatus: planned` 的 ZIP 骨架，在运行时完成前拒绝一键安装；原版植物替换同样不会生成误导性安装包。
