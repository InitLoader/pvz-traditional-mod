...
动画制作器的大工程性能不能依赖每次播放刷新时遍历全部历史帧和全部时间轴单元格。轨道的省略字段继承结果使用前缀缓存，并由帧属性或集合变更精确失效；图片图集裁剪按符号与子帧缓存；动作范围、动作轨道、曲线显示和关键帧索引在没有编辑时复用；时间轴按 `ScrollViewer` 可见范围裁剪绘制。播放使用高优先级时钟累计真实经过时间并按应播放帧数追赶，因此一次 UI 绘制延迟不会让后续动画永久落后。
