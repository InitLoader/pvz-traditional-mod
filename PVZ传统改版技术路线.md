...
完整轨道复制是另一条工作流：`Ctrl+Shift+C` 记录选中轨道的全部帧、曲线、图片资源和子帧布局，切换到同一动画或另一动画后用 `Ctrl+Shift+V` 创建独立新轨道与唯一图片符号，不与原轨道共用可变数据。轨道左侧的眼睛只控制编辑器可见性，不删除图层、不改变 Raw/compiled 导出，状态随 `.pvza` 保存并可撤销。
