...
导出 Raw/compiled、生成 ZIP 和一键安装共用发布确认阶段。确认模型先复制工程的实体类型、字符串/数字 ID、名称、模板、初始动作和战斗属性，用户可修改后一次性写回并进入撤销历史；缺少名称、非法 ID、不存在的初始动作、越界模板或数值会阻止继续。僵尸骨架通过 `Zombie_*` 及 `anim_bucket/anim_cone/anim_screendoor` 等明确轨道识别，禁止误设为植物；字符串 ID/名称仍带相反实体语义时要求改名。

一键安装必须先做跨配置冲突检查：同一动画 ID 已由不同 `carrierReanimation` 使用，或植物与僵尸配置引用同一动画 ID 时，禁止覆盖并要求新 ID。图片资源 ID 统一限制为 `[A-Za-z0-9_]{1,64}`；输入符号含特殊字符或超长时使用安全前缀加稳定 SHA-256 短哈希，动画 `images` 映射与 `textures.jsonc` 必须引用同一个结果。写入动画、图片、资源配置和实体配置前记录文件快照，完成后按 DLL 规则校验整个注册表；任何异常按逆序恢复，避免出现“动画已覆盖、实体配置只写了一半”的存档崩溃状态。
