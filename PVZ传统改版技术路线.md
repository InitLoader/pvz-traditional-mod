...
插入空帧不能依赖 Reanimation 的字段继承，否则中间帧会一直保持上一关键帧而造成运动冻结。插入/删除整帧前后由曲线服务维护关键点索引并重新烘焙 `x/y/kx/ky/sx/sy/a`；旧 compiled 缺少编辑器曲线时，先从非动作轨道的显式运动值重建曲线。图片符号、图片子帧和动作显示标记仍保持离散，避免自动插值破坏换图与动作范围。
