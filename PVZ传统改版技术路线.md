...
时间轴和曲线编辑器按 `B` 进入框选，可跨多条轨道或多个曲线通道批量选择、移动和删除。关键帧横向移动改为逐格交换：拖过已有关键帧时只改变顺序，不删除目标帧，因此不会出现往一个方向拖动后沿途关键帧全部合并的问题。画布按 `G/R/S` 进入鼠标移动、旋转或缩放，左键确认、右键/`Esc` 取消；旋转角度跟随鼠标绕图片视觉中心变化，并通过同步补偿 `x/y` 保持中心不漂移。
