...
完整轨道复制是另一条工作流：`Ctrl+Shift+C` 记录选中轨道的全部帧、曲线、图片资源和子帧布局，切换到同一动画或另一动画后用 `Ctrl+Shift+V` 创建独立新轨道与唯一图片符号，不与原轨道共用可变数据。轨道左侧的眼睛只控制编辑器可见性；锁定状态禁止属性、画布、时间轴与曲线写操作；二者都不改变 Raw/compiled 导出，状态随 `.pvza` 保存并可撤销。时间轴还显示按 FPS 换算的独立秒尺，每条轨道显示首个有效图片的缩略图，属性检查器显示当前图片、行列布局和来源路径。
