...
编辑历史由 `EditHistoryService` 和工程深拷贝服务维护，明确保留最近 100 步。画布移动/旋转/缩放、K 帧、补间、插入/删除帧、轨道/动作增删、实体属性、动作参数、FPS 和图片导入全部进入同一会话历史；`Ctrl+Z` 连续撤销，`Ctrl+Y` 或 `Ctrl+Shift+Z` 按原顺序连续恢复，撤销后新编辑会建立新分支。

时间轴和曲线编辑器的框选为常驻交互：左键命中关键帧时选择或拖动关键帧，命中空白位置时直接开始框选，不需要先按快捷键；可跨多条轨道或多个曲线通道批量选择、移动和删除。关键帧横向移动改为逐格交换：拖过已有关键帧时只改变顺序，不删除目标帧，因此不会出现往一个方向拖动后沿途关键帧全部合并的问题。画布按 `G/R/S` 进入鼠标移动、旋转或缩放，左键确认、右键/`Esc` 取消；旋转角度跟随鼠标绕图片视觉中心变化，并通过同步补偿 `x/y` 保持中心不漂移。

`_ground` 必须作为原版语义的定位/速度轨道单独处理，不能因为没有贴图而从画布过滤。编辑器以青色路径、当前点和相邻帧方向箭头显示它，允许直接拖动 X/Y，并在属性检查器显示原版 `GetTrackVelocity = ΔX × 0.01 × mAnimRate` 的像素/更新结果及 `ΔX × mAnimRate` 的像素/秒换算。动作局部视图按标记轨道切开 X 重置边界，播放和秒尺使用动作 `rate`；完整视图则按当前帧自动匹配所在动作，避免把 `anim_walk` 末尾到 `anim_walk2` 起点的重置误报为瞬移速度。

插入空帧不能依赖 Reanimation 的字段继承，否则中间帧会一直保持上一关键帧而造成运动冻结。插入/删除整帧前后由曲线服务维护关键点索引并重新烘焙 `x/y/kx/ky/sx/sy/a`；删除起点和终点之间的中间关键帧时，也先保存两侧运动上下文，再移除关键点并重烘焙整段，不能保持到终点才瞬移。旧 compiled 缺少编辑器曲线时，必须在清空待删帧前从非动作轨道的显式运动值重建曲线。图片符号、图片子帧和动作显示标记仍保持离散，避免自动插值破坏换图与动作范围。

动作标记轨道的 `f` 不能参与普通 Bezier/线性补间。`f=0` 表示动作仍可见，`f=-1` 表示从该帧隐藏；如果烘焙出 `-0.1/-0.2`，动作范围检测会提前结束，而部件运动仍按完整区间播放。编辑器现在对纯 `anim_*` 标记强制常量插值，插帧时同步移动标记终点与运动终点，并在加载旧工程或旧导出文件时清理 `(-1, 0)` 的错误过渡值。

轨道关键帧复制使用编辑器内部剪贴板：同一轨道的框选点或整轨点以最早帧为相对原点，复制 Transform 曲线关键点、插值类型、Bezier 手柄及离散图片/文字；粘贴到当前轨道时以当前帧为新原点，覆盖碰撞点并重新烘焙目标轨道。已有曲线的逐帧烘焙结果不得当成独立关键点重复复制，只有旧格式中没有曲线元数据的显式值才转换为关键点；粘贴整体进入一次 `Ctrl+Z/Ctrl+Y` 历史。

完整轨道复制是另一条工作流：`Ctrl+Shift+C` 记录选中轨道的全部帧、曲线、图片资源和子帧布局，切换到同一动画或另一动画后用 `Ctrl+Shift+V` 创建独立新轨道与唯一图片符号，不与原轨道共用可变数据。轨道左侧的眼睛只控制编辑器可见性；“常显”图钉让防具、肢体或附属轨道绕过普通僵尸预览的可选装备过滤；锁定状态禁止属性、画布、时间轴与曲线写操作。三者都不改变 Raw/compiled 导出，状态随 `.pvza` 保存并可撤销。时间轴按 FPS 换算的秒尺和列说明固定在滚动视口顶部，每条轨道显示首个有效图片的缩略图，属性检查器显示当前图片、行列布局和来源路径。

动画制作器的大工程性能不能依赖每次播放刷新时遍历全部历史帧和全部时间轴单元格。轨道的省略字段继承结果使用前缀缓存，并由帧属性或集合变更精确失效；图片图集裁剪按符号与子帧缓存；动作范围、动作轨道、曲线显示和关键帧索引在没有编辑时复用；时间轴按 `ScrollViewer` 可见范围裁剪绘制，并必须监听每个所属滚动视图的 `ScrollChanged`，否则偏移改变后新露出的区域会保持旧空白直到其他输入触发重绘。播放使用高优先级时钟累计真实经过时间并按应播放帧数追赶，因此一次 UI 绘制延迟不会让后续动画永久落后。
...
编辑器工作区采用可序列化二叉拆分树。每个叶区域可在动画视图、时间轴、曲线编辑器、资源浏览器和属性检查器之间切换，横纵分隔条使用 10 像素抓取区、保存比例并随窗口自适应，可直接把时间轴拉高或把资源/属性区拉宽；区域角落或横/纵拆分按钮可继续分区，独立窗口共享同一工程和撤销历史。内置动画制作、双动画视图、双时间轴、曲线动画和专注动画预设，布局随工程传递。
...
原版图片的“编辑器可见”与“发布所有权”必须分离。打开原版 compiled 后，所有实际引用符号仍进入图片资源面板并显示缩略图、解析路径和原版标记；轨道缩略图在资源索引完成后重新生成。Reanimation 当前帧省略 `i` 时，属性栏显示继承后的实际符号并标明来源，但不把只读显示写回为显式关键帧。`originalImageReferences` 继续决定发布过滤，因此这些编辑器可见的原版图片不会进入 `pvzmod/images`、贴图注册或动画图片映射。

导出 Raw/compiled、生成 ZIP 和一键安装共用发布确认阶段。确认模型显式区分“替换原版动画”和“新增实体”：从游戏 `compiled/reanim` 直接打开原版动画时自动进入替换模式，识别到 `Zombie_*` 及 `anim_bucket/anim_cone/anim_screendoor` 等主体轨道时自动选择僵尸，不再要求先建立伪新增僵尸工程。替换模式只要求目标原版 ID，隐藏新增数字 ID 与战斗数值；新增模式才确认字符串/数字 ID、名称、模板、初始动作和战斗属性。用户修改后一次性写回并进入撤销历史；缺少名称、非法 ID、不存在的初始动作、越界模板或数值会阻止继续。

一键安装必须先做接入模式与跨配置冲突检查：当前允许模板兼容型新增植物，以及替换原版植物或僵尸主体动画；真正新增僵尸在对应运行时完成前拒绝安装。替换植物只字段级合并目标 `plants.<id>.animationId`，替换僵尸只合并 `zombies.<id>.animationId`，保留既有生命、攻击、防具、卡片和其他扩展字段，不生成伪新增实体配置。同一动画 ID 已由不同 `carrierReanimation` 使用，或植物与僵尸配置引用同一动画 ID 时，禁止覆盖并要求新 ID。图片资源 ID 统一限制为 `[A-Za-z0-9_]{1,64}`；输入符号含特殊字符或超长时使用安全前缀加稳定 SHA-256 短哈希。原版图片符号由 schema 5 的 `originalImageReferences` 标记，便携工程可嵌入预览副本，但打包只复制动画实际引用的 Mod 图片；原版符号省略 `images` 映射并由运行时从原 Definition 解析。写入动画、图片、资源配置和实体配置前记录文件快照，完成后按 DLL 规则校验整个注册表；任何异常按逆序恢复，避免出现“动画已覆盖、实体配置只写了一半”的存档崩溃状态。完整模式和迁移规则见 `modding/ANIMATION_REPLACE_AND_ADD_MODES.md`。
...
僵尸外部主体动画不能在 Holder 重建时丢失原版的实例分层状态。运行时在释放旧 TrackInstance 前按轨道名快照 `RenderGroup`、裁剪和消失截断标志，初始化新 Definition 后再恢复，让路障、铁桶、铁门、旗帜和泳圈等仍由原版 `Zombie::SetupReanimLayers` 选择。编辑器对普通僵尸及其另存工程自动隐藏未启用装备；选中某装备轨道时会临时显示供编辑，开启该轨道的编辑器“常显”图钉后则在切换其他轨道或动作时继续显示，便于同时校对身体、防具和附属肢体。这个标记只属于 `.pvza` 编辑状态，不干预游戏运行时分层。
...
自定义植物 ID 从 1000 开始，与原版模板 ID 分离。卡片默认解锁，选择后将 `templatePlantId` 与自定义逻辑 ID 一起写入种子包；前者只复用原版动画、动作和目标选择，后者负责查找自己的价格、冷却、生命、射速、首发延迟、连发数、子弹类型和伤害。因此原版豌豆射手和自定义豌豆套壳可以同时选用，且数值互不覆盖。动画编辑器用 `PlantTemplateCatalog` 全局管理原版植物 `SeedType 0–52`，其中只有标准植物 `0–48` 可作为当前运行时模板；`49–52` 是坚果保龄球、禅境花园或特殊模式专用类型。完整映射见 `modding/PvZAnimationStudio/PLANT_TEMPLATE_IDS.md`。
