...
- `F11` — 独立中文动画制作器：按原版矩阵/注册点/子帧规则预览、可拖动/删除轨道关键帧、带彩色通道图例与可拉 Bezier 手柄的曲线编辑器、Blender 式可拆分工作区和独立窗口、完整 `Ctrl+Z`/`Ctrl+Y`、任意目录 compiled 文件头识别、内嵌图片/曲线/动作/属性/布局的 `.pvza` 单文件工程、JSONC/ZIP 打包和一键安装。
...
- `P06` — 动画制作器会动态保留所有 `anim_*` 动作并支持眨眼/特殊动作编辑，已有可持久化 Bezier 曲线手柄和完整撤销/恢复；当前尚未实现骨骼 IK、多关键点框选、曲线修改器和音频轨。
