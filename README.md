...
- `P05` — 外部动作资源已完成 Raw `.reanim` 与原版 PC `.reanim.compiled` 自动读取、安全解析、ABI Definition 构建、自定义植物主体注入，以及单一外部动画 Definition 的关卡存档恢复；动作替换和事件元数据可制作、校验、保存和导出，但安全的植物局部运行时路由、多个外部 Definition、独立附件与僵尸动画仍未接入。
...
- `T05` — 自定义僵尸、植物多个独立外部附属 Reanimation、原创音效和资源热加载管线。
