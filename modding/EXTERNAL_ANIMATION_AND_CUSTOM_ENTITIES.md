...
2. 在中文分层画布和动作局部时间轴中设置 K 帧；时间轴菱形可左右拖动或独立删除。曲线编辑器按颜色区分位移、旋转、缩放、图片子帧和透明度，可拖关键点及 Bezier 手柄，并把结果烘焙回原版逐帧值。画布使用与原版一致的矩阵、左上注册点和图片子帧语义。
...
8. 需要 Adobe Animate/XFL、骨骼 IK、多关键点框选或音频轨等进阶功能时，可使用 PopStudio、Twinning、EffectViewer 或 JSFL 流程转为 Raw，再回到本工具检查和打包。
