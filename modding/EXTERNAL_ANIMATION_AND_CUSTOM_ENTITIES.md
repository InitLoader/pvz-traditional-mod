...
2. 在中文分层画布和动作局部时间轴中设置 K 帧；时间轴或曲线区按 `B` 后可跨轨道/通道框选，批量拖动并用 `Delete`/`Backspace` 删除。拖动跨过已有关键帧时按顺序交换，禁止自动合并吞帧。曲线编辑器按颜色区分位移、旋转、缩放、图片子帧和透明度，可拖关键点及 Bezier 手柄，并把结果烘焙回原版逐帧值。画布使用与原版一致的矩阵、左上注册点和图片子帧语义；`G/R/S` 鼠标变换中旋转围绕图片视觉中心。
