...
4. 导入 PNG 或原版 JPG + `_.png` 灰度透明蒙版后，工具可自动生成图片、动作、植物/僵尸配置片段以及 Raw/compiled，并可打包 ZIP 或一键合并 JSONC。
5. 所有编辑共用完整会话历史：`Ctrl+Z` 可连续撤销，`Ctrl+Y`/`Ctrl+Shift+Z` 可连续恢复，撤销后进行新编辑会清除旧恢复分支。
6. 需要 Adobe Animate/XFL、骨骼或曲线手柄等进阶功能时，可使用 PopStudio、Twinning、EffectViewer 或 JSFL 流程转为 Raw，再回到本工具检查和打包。
7. 完全退出并重新启动游戏，通过 `pvzmod/logs/pvzmod.log` 检查解析和交叉校验。
