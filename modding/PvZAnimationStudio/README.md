...
- `Q/W/E/R` 选择、移动、旋转和缩放操纵器；红/蓝轴、旋转环和等比缩放手柄直接作用于画布部件。
- 无步数截断的会话历史：`Ctrl+Z` 逐步撤销，`Ctrl+Y` 或 `Ctrl+Shift+Z` 按原顺序逐步恢复；画布、K 帧、补间、帧/轨道/动作、工程字段、数值、FPS 和图片导入都进入同一历史。
- 鼠标拖动部件，编辑 `x/y/kx/ky/sx/sy/f/a/i/font/text`。
- 从当前关键帧到右侧下一个关键帧生成线性或平滑补间；补间会烘焙位移、旋转/斜切、缩放和透明度。
- 时间轴菱形关键帧可直接左右拖动，或用工具栏及 `Shift+←/→` 逐帧移动；`Delete`/“删除关键帧”只删除当前轨道关键帧，“删除整帧”才会压缩所有轨道的时间。
- 新增 Blender 风格曲线编辑器：X/Y 位移、双旋转轴、双缩放、图片子帧和透明度使用固定不同颜色，顶部图例明确标注线条用途；关键点可拖动帧和值，左右手柄可直接拉 Bezier 曲线，并支持自动、对齐、自由、矢量手柄以及 Bezier、线性、常量插值。
...
| `W` / `G` | 移动；拖中心或红/蓝轴 |
| `E` | 旋转；拖黄色圆环 |
| `R` / `S` | 缩放；红色 X、蓝色 Y、黄色等比手柄 |
| 方向键 / `Shift`+方向键 | 1 / 10 像素微调 |
| `K` / `Shift+K` | 设置 / 清除当前 K 帧 |
| `Ctrl+Z` | 逐步撤销，直到本次打开或新建工程时的初始状态 |
...
- 曲线在编辑器中保留 Bezier 关键点与手柄，同时烘焙为原版可读取的逐帧数值；当前还没有骨骼 IK、多关键点框选、曲线修改器和音频时间轴。
