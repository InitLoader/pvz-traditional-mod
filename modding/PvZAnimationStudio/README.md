...
- 新增 Blender 风格曲线编辑器：X/Y 位移、双旋转轴、双缩放、图片子帧和透明度使用固定不同颜色，顶部图例明确标注线条用途；在曲线空白位置随时按住左键拖动即可跨多个通道框选，批量改变帧和值或批量删除。关键点跨越时采用交换重排而非自动合并；左右手柄可直接拉 Bezier 曲线，并支持自动、对齐、自由、矢量手柄以及 Bezier、线性、常量插值。
