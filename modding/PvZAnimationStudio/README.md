...
- 时间轴菱形关键帧可直接左右拖动，或用工具栏及 `Shift+←/→` 逐帧移动；拖过已有关键帧时交换顺序，不会沿途合并或吞掉帧。时间轴按 `B` 后框选可跨多条轨道选择关键帧，`Ctrl`/`Shift` 追加选择，随后可批量拖动或用 `Delete`/`Backspace` 删除；“删除整帧”才会压缩所有轨道的时间。
- 新增 Blender 风格曲线编辑器：X/Y 位移、双旋转轴、双缩放、图片子帧和透明度使用固定不同颜色，顶部图例明确标注线条用途；按 `B` 可跨多个通道框选，批量改变帧和值或批量删除。关键点跨越时采用交换重排而非自动合并；左右手柄可直接拉 Bezier 曲线，并支持自动、对齐、自由、矢量手柄以及 Bezier、线性、常量插值。
...
- 时间轴和曲线区按 `B` 后拖框选择；`Ctrl`/`Shift` 可保留原选择。选中的菱形/曲线点为橙色，可一起拖动，并可用 `Delete` 或 `Backspace` 批量删除。该交互对应 Blender 官方手册的 [Graph Editor Box Select](https://docs.blender.org/manual/en/latest/editors/graph_editor/introduction.html#select-menu)。
...
| `B`（时间轴/曲线区） | 框选多个关键帧；`Ctrl`/`Shift` 追加选择 |
