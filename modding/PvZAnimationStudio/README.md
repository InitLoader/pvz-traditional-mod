...
- 在两个关键帧之间插入或删除整帧时，自动重新烘焙位移、旋转、缩放和透明度曲线；新增空帧会得到连续的中间值，不再继承上一帧造成动作冻结。旧 compiled 没有曲线元数据时，会先从显式运动帧建立曲线；图片切换和动作显示标记不会被擅自平滑。
...
- 省略字段表示继承前一帧；设置 K 帧、生成补间以及插入/删除整帧后的曲线重烘焙会写入显式运动数值。
