...
`PvZAnimationStudio` 应增加“创建并绑定技能”向导：选择植物/僵尸和模板、填写技能 ID、生成 Lua、自动写入绑定、启动语法检查并打开技能文件。向导只自动完成机械步骤，生成结果仍是普通文本文件，不形成只能由编辑器读取的私有格式。
...
- 打包器负责生成/更新 manifest、检查重复 ID、能力兼容、无用脚本和缺失绑定，并可把检查结果显示在动画制作器中。
...
| 20–35 分钟 | 用“创建并绑定技能”向导生成自己的技能 | 自动创建 Lua、绑定实体并跳到待编辑行 |
...
- `PvZAnimationStudio` 的行为脚本页：创建模板、绑定实体、语法检查、热重载、错误列表、事件检查器和状态查看器。
...
开发模式中的 Lua 省略元数据时默认使用 `apiVersion: 1`，并在实际调用不存在的能力时给出精确错误。`PvZAnimationStudio` 和打包器负责根据 JSON 规则、Lua 模板、配方依赖、DLL manifest 和验证结果生成统一索引；发布包中的每个模块必须拥有明确的 API/ABI 版本和 `requires`。新手可以先写技能，准备分享时再由工具完成兼容性声明。
...
- 在 `PvZAnimationStudio` 接入“创建并绑定技能”、错误跳转、热重载提示和事件检查器。
...
- 动画制作器向导可以在一次操作中创建 Lua、写入绑定、验证并打开待编辑位置；手工文件路径仍完整可用。
